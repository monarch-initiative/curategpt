...
class GOCAMPredictor:
    ...
    def __post_init__(self):
        ...
        self.gocam_wrapper = GOCAMWrapper(include_standard_annotations=self.include_standard_annotations)
    ...
    def predict(self, gocam: Dict[str, Any], stub: Dict[str, Any], num_examples=0) -> Dict[str, Any]:
        """
        ...
        examples = []
        if num_examples:
            for result, _score, _meta in self.store.search(gocam_yaml, limit=num_examples+1, collection=self.collection_name):
...
database_type_option = click.option("--database_type", default="chromadb", help="The type of database to use")
database_path_option = click.option("--database_path", default="gocam", help="The path to the database")
collection_name_option = click.option("--collection_name", default="gocam", help="The name of the collection")
model_name_option = click.option("--model-name", default="gpt-4o", help="The name of the model")
num_examples_option = click.option("--num-examples", "-N",
              type=click.INT,
              default=0, show_default=True, help="Number of RAG examples to generate")
...
@click.option("--include-standard-annotations/--no-include-standard-annotations",
              default=False,
              show_default=True,
...
def predict(gocam_id, pmid, model_name, database_type, database_path, collection_name, num_examples, output, include_standard_annotations, prior_results_file):
    ...
    if gocam_id:
        ...
        gocam_ids = [x["id"] for x, _s, _m in predictor.store.find({}, collection=collection_name, limit=9999)]
    ...
    for gocam_id in gocam_ids:
        ...
        for pmid in pmids:
            ...
            if not expected:
                raise ValueError(f"Activity with reference {pmid} not found in GO-CAM model {gocam_id} // {refs}")
            ...
            print("## OUTCOME")
            print(yaml.dump(outcome.model_dump(exclude={"predicted", "expected", "has_standard_annotation"}), sort_keys=False))
            output_file.write("---\n")
